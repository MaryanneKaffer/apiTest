...
interface InputProps {
  ...
  size?: (arg0: string) => void;
  description?: (arg0: string) => void;
}

export default function OrderInput({ name, width, id, disabled, fixedValue, type, noBorder, value, total, size, description }: InputProps) {
  const date = new Date();
  ...
  return (
    <>
      {type === "autoComplete" && (
        <Autocomplete maxLength={3} onKeyDown={(e) => { if (!/[0-9]|Backspace|Tab|ArrowLeft|ArrowRight/.test(e.key)) { e.preventDefault(); } }} onSelectionChange={(value) => { setInputValue(value as string); const selectedBag = bags.find(bag => bag.code === value); if (selectedBag && size) size(selectedBag.size); if (selectedBag && description) description(selectedBag.description) }} className={`h-[38px] ${width}`} radius="none" style={{ padding: '0' }} classNames={{ base: 'border-r-2 border-b-2 border-gray-600', clearButton: 'hidden', selectorButton: 'hidden', popoverContent: 'p-0', }} >
          {bags.map((value) => (
        ...
        type === "description" && (
          <Input
            ...
            classNames={{
              ...
              input: `${id !== "description" && 'text-center'}`,
            }}
          />
        )
      }
